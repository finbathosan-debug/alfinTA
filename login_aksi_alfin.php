...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['loginAlfin'])) {
    ...
    $dataAlfin = mysqli_prepare($koneksiAlfin, "SELECT username_alfin, password_alfin FROM pengguna_alfin WHERE username_alfin = ? LIMIT 1");
    if ($dataAlfin) {
        ...
        if (mysqli_stmt_num_rows($dataAlfin) === 1) {
            mysqli_stmt_bind_result($dataAlfin, $dbUsername, $dbPasswordHash);
